...
- **FR-008**: システムは、処理の成否（設定変更の完了、またはエラーによる中断）をFirebase Cloud Messaging (FCM) やローカルログを用いて記録・通知できなければならない。ログには、SOCの計算結果だけでなく、その根拠となった天気予報とスケジュール情報（Context Logging）を含めなければならない。
- **FR-009**: システムは、任意のタイミングでSOC設定を強制実行できる手動オーバーライド機能を提供しなければならない。
- **FR-010**: システムは、アプリの稼働状況を定期的に報告するハートビート機能（Device Health）を備えなければならない。
